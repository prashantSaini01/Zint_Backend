...
// export const rejectInvite = async (event) => {
//   try {
//     const { token } = event.pathParameters;
//     const userId = event.requestContext.authorizer.principalId;
...
//     return {
//       statusCode: 200,
//       body: JSON.stringify({ message: 'Invitation rejected successfully' }),
//     };
//   } catch (error) {
...
export const rejectInvite = async (event) => {
  try {
    ...
    return formatJSONResponse(200, { message: 'Invitation rejected successfully' });
  } catch (error) {
    console.error('Reject invite error:', error);
    return createErrorResponse(500, 'Failed to reject invitation');
  }
...
export const deleteInvite = async (event) => {
  try {
    ...
    return formatJSONResponse(200, { message: 'Invitation cancelled successfully' });
  } catch (error) {
    console.error('Delete invite error:', error);
    return createErrorResponse(500, 'Failed to cancel invitation');
  }
...
export const getBoardInvites = async (event) => {
  try {
    ...
    const invites = await Invite.find({ boardId }).select('-__v -updatedAt');
    return formatJSONResponse(200, invites);
  } catch (error) {
    console.error('Get board invites error:', error);
    return createErrorResponse(500, 'Failed to retrieve board invites');
  }
