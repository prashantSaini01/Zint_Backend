...
export const createlist = async (req, res) => {
  try {
    ...
    // Determine order based on the number of existing lists if not provided
    if (order === undefined) {
      const listsInBoard = await List.find({ boardId });
      order = listsInBoard.length; // Set order to the next available position
    }
...
export const getlistsbyid = async (req, res) => {
  try {
    ...
    // Fetch the list by ID (only if active)
    const list = await List.findOne({ _id: id, deleted: false });
